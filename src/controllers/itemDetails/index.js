...
function renderGenres(page, item, context = inferContext(item)) {
    ...
    genresLabel.innerHTML = globalize.translate(
...
function renderWriter(page, item, context) {
    ...
    writersLabel.innerHTML = globalize.translate(
...
function renderDirector(page, item, context) {
    ...
    directorsLabel.innerHTML = globalize.translate(
...
function renderStudio(page, item, context) {
    ...
    if ([BaseItemKind.BoxSet, BaseItemKind.Playlist].includes(item.Type))
        return;
    ...
    studiosLabel.innerText = globalize.translate(
...
function renderChildren(page, item) {
    ...
    let fields =
        "ItemCounts,PrimaryImageAspectRatio,CanDelete,MediaSourceCount,People";
    const query = {
        ParentId: item.Id,
        Fields: fields,
    };
...
function renderPlaylistItems(page, item) {
    import("../../scripts/playlistViewer").then(
